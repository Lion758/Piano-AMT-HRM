...
class TokenSize(Enum):
    ...
    PEDAL_ON = 1  # Single token for pedal on
    PEDAL_OFF = 1  # Single token for pedal off
...
class TokenBase():
    ...
    def get_value(cls, token_val:int) -> int:
        ...
        if (value < 0) or (value >= cls.__size):
            print("Token value out of bound: %s (%d, %d), bound: (%d, %d)"%(cls.get_type_name(), token_val, value, cls.begin_index, cls.__size))
...
class TokenPedal(TokenBase): # 12
    name = "Pedal"
class TokenPedalOn(TokenBase):
    name = "PedalOn"
class TokenPedalOff(TokenBase):
    name = "PedalOff"
...
class SymbolicMusicTokenizer:
    ...
    def _initialize_token_dicts(self):
        ...
        TokenPedalOn.set_size(TokenSize.PEDAL_ON.value)
        TokenPedalOff.set_size(TokenSize.PEDAL_OFF.value)
        ...
        self.token_type_list.append(TokenPedalOn)
        self.token_type_list.append(TokenPedalOff)
    ...
    def midi_to_dataframe(self, midi_path: str, midi:symusic.Score = None) -> pd.DataFrame:
        """
        Args:
            midi_path
        Returns: 
        ...
        for track in midi.tracks:
            ...
            # Add pedal events (PedalOn at pedal start, PedalOff at pedal end)
            if len(pedals["time"]) > 0:
                df_pedal_on = pd.DataFrame({
                    "type": "PedalOn",
                    "type_id": 0,  # Pedal sorts before notes at same time
                    "pitch": -1,
    ...
    def tokenize_dataframe(self, df_data:pd.DataFrame, sequence_type = "performance") -> List[str]:
        """
        Args:
        ...
        for index, row in df_data.iterrows():
            ...
            # Pedal events: [Onset, PedalOn/PedalOff, BLANK]
            if row["type"] == "PedalOn":
                onset_token = TokenOnset(onset)
                pedal_on_token = TokenPedalOn(0)
                append_token(cp_word, onset_token)
                append_token(cp_word, pedal_on_token)
                cp_word.append(self.BLANK)

            elif row["type"] == "PedalOff":
                onset_token = TokenOnset(onset)
                pedal_off_token = TokenPedalOff(0)
                append_token(cp_word, onset_token)
                ...
                if row['type'] == 'NoteOn':
                    ...
                    velocity_downsample = int( np.floor(velocity/VELOCITY_DOWN_SAMPLING) )
    ...
    def detokenize(self, tokens: List[str], offsets_sec) -> list:
        ...
        for i in range(num_tokens):
            ...
            if TokenOnset.is_instance(token):
                curr_onset_sec = TokenOnset.get_value(token) / ONSET_SEC_UP_SAMPLING + offsets_sec
            ...
            # Pedal On
            elif TokenPedalOn.is_instance(token):
                pedal_event = {
                    "pitch": -1,
                    "time": curr_onset_sec,
                    "type": "PedalOn",
                ...
                midi_event_list.append(pedal_event)

            # Pedal Off
            elif TokenPedalOff.is_instance(token):
                pedal_event = {
                    "pitch": -1,
                    "time": curr_onset_sec,
                    "type": "PedalOff",
            ...
            # Velocity
                ...
                if len(midi_event_list) > 0:
                    ...
                    if midi_event_list[-1]["velocity"] != 0: # Only set if it's a NoteOn event
    ...
    def notes_to_midi_events(self, df: pd.DataFrame) -> List[Dict[str, Union[int, float]]]:
        ...
        if len(df_pedal_off) > 0:
            ...
            event_frames.append(df_pedal_off)

        df_midi_events = pd.concat(event_frames, ignore_index=True)
        df_midi_events = df_midi_events.sort_values(by=["onset_sec", "type_id", "pitch"], ascending=[True, True, True])
        
        return df_midi_events
    
    def midi_events_to_notes(self, midi_event_list: List[Dict[str, Union[int, float]]]) -> List[Dict[str, Union[int, float]]]:
        """
        Args:
            midi_event_list: List of MIDI event dictionaries.
        Returns:
            midi_note_list: List of MIDI note data dictionaries.
            pedal_event_list: List of pedal event dictionaries with 'time' and 'type' keys.
        """
        midi_note_list = []
        pedal_event_list = []
        ...
        # Separate pedal events
        df_pedals = df[df["type"].isin(["PedalOn", "PedalOff"])]
        for _, event in df_pedals.iterrows():
            pedal_event_list.append({"time": event["time"], "type": event["type"]})

        # Process note events only (pitch >= 0)
        df_notes = df[df["pitch"] >= 0]
        if len(df_notes) == 0:
            return midi_note_list, pedal_event_list
        ...
        for (pitch,), group in grouped:
            for i in range(len(group)):
                ...
                if i + 1 < len(group):
                    dur_sec = group.iloc[i + 1]['time'] - onset_sec
                else:
                    dur_sec = 0.05
                dur_sec = np.clip(dur_sec, 0.02, 20.0)
                midi_note_data = {
                    "pitch": pitch,
                    "onset": onset_sec,
                    "duration": dur_sec,
    ...
    def save_midi(self, midi_note_list, midi_path, performance_downbeats = [], performance_timesignatures = [], pedal_event_list = []):
        ...
        time_signature_list = []
        time_signature_list = [{"time": 0.0, "numerator": 4, "denominator": 4, "bar_dur_q": 4.0}]
        
        for ts_data in performance_timesignatures:
            t,ts_str = ts_data["time"], ts_data["value"]
            if ts_str != prev_ts_str:
                bar_dur_q = music21.meter.TimeSignature(ts_str).barDuration.quarterLength
                numerator, denominator = ts_str.split("/")
                numerator = int(numerator)
                denominator = int(denominator)
                midi.time_signature_changes.append(pretty_midi.containers.TimeSignature(numerator, denominator, float(t)))
                prev_ts_str = ts_str
                if t <= 0.0:
                    time_signature_list = [{"time": 0.0, "numerator": numerator, "denominator": denominator, "bar_dur_q": bar_dur_q}]
                else:
                    time_signature_list.append({"time": t, "numerator": numerator, "denominator": denominator, "bar_dur_q": bar_dur_q})
        ...
        # Write pedal events as CC64 messages
        if len(pedal_event_list) > 0 and len(midi.instruments) > 0:
            for pedal_event in pedal_event_list:
                cc_value = 127 if pedal_event["type"] == "PedalOn" else 0
                cc = pretty_midi.ControlChange(number=64, value=cc_value, time=pedal_event["time"])
                midi.instruments[0].control_changes.append(cc)
