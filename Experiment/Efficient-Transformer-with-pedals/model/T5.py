...
def make_pedal_head(pedal_head_type, emb_dim, kernel_size=7, hidden=64, dropout=0.1):
    pedal_head_type = str(pedal_head_type or "linear").strip().lower()
    if pedal_head_type == "linear":
        return nn.Linear(emb_dim, 1)
    if pedal_head_type == "temporal_conv":
        return TemporalConvPedalHead(
            emb_dim,
            kernel_size=kernel_size,
            hidden=hidden,
            dropout=dropout,
        )
    raise ValueError(f"Unsupported pedal_head_type {pedal_head_type!r}; expected 'linear' or 'temporal_conv'.")


def apply_pedal_head(head, encoder_outputs):
...
class Transformer(nn.Module):
    def __init__(self, config):
        ...
        pedal_head_type = getattr(config, "pedal_head_type", "linear")
        ...
        self.pedal_frame_head = make_pedal_head(
            pedal_head_type,
            config.emb_dim,
        ...
        self.pedal_onset_head = make_pedal_head(
            pedal_head_type,
            config.emb_dim,
        ...
        self.pedal_offset_head = make_pedal_head(
            pedal_head_type,
            config.emb_dim,
    ...
    def forward(self, 
        ...
        res_dict["pedal_frame_logits"] = apply_pedal_head(self.pedal_frame_head, encoder_outputs)
        res_dict["pedal_onset_logits"] = apply_pedal_head(self.pedal_onset_head, encoder_outputs)
        res_dict["pedal_offset_logits"] = apply_pedal_head(self.pedal_offset_head, encoder_outputs)
        ...
        for i in tqdm(range(0, target_seq_length, pred_step), desc="Generating tokens (rank %d)" % global_rank):
            ...
            if hasattr(self.config, "hybrid_global_local_cross_attn") and self.config.hybrid_global_local_cross_attn:
                ...
                for b in range(batch_size):
                    ...
                    elif Tokenizer.TokenPedalOn.is_instance(token_i):
                        token_type_i = Tokenizer.TokenPedalOn
                    elif Tokenizer.TokenPedalOff.is_instance(token_i):
                        token_type_i = Tokenizer.TokenPedalOff
                    else:
                        token_type_i = "Other"
                    if i % 3 == 0:
                        dest_token_type_i = (Tokenizer.TokenOnset,)
                    elif i % 3 == 1:
                        dest_token_type_i = (Tokenizer.TokenPitch, Tokenizer.TokenNoteOff, Tokenizer.TokenPedalOn, Tokenizer.TokenPedalOff)
                    elif i % 3 == 2:
                        prev_token_i = gen_tokens[b, i-1].item()
                        if (Tokenizer.TokenNoteOff.is_instance(prev_token_i) or Tokenizer.TokenPedalOn.is_instance(prev_token_i) or Tokenizer.TokenPedalOff.is_instance(prev_token_i)) and token_i != TOKEN_BLANK:
                            # NoteOff and pedal events should be followed by TOKEN_BLANK
                            print(f"Warning: The generated token {token_i} at batch idx {b} position {i} is a velocity token, but the previous token is a NoteOff/Pedal. The velocity should be {TOKEN_BLANK}, but got {token_i}.")
            ...
            if hasattr(self.config, "hybrid_global_local_cross_attn") and self.config.hybrid_global_local_cross_attn:
                ...
                elif i % 3 == 1: # Pitch token, NoteOn, NoteOff, PedalOn, or PedalOff
                    begin, end = Tokenizer.TokenPitch.get_bound()
                    probs[:, :, begin:end] += 1
                    begin, end = Tokenizer.TokenNoteOff.get_bound()
                    probs[:, :, begin:end] += 1
                    begin, end = Tokenizer.TokenPedalOn.get_bound()
                    probs[:, :, begin:end] += 1
                    begin, end = Tokenizer.TokenPedalOff.get_bound()
                    probs[:, :, begin:end] += 1
