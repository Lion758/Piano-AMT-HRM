...
from train import (
    ...
    should_strip_legacy_linear_pedal_head_layers,
...
def _load_model_if_needed(config: OmegaConf) -> None:
    ...
    legacy_pedal_head_layers = (
        remove_legacy_linear_pedal_head_layers(state_dict)
...
def run_inference(config: OmegaConf) -> None:
    ...
    for i in tqdm(range(num_batches), desc="Inference"):
        ...
        for b in range(encoder_inputs_i.shape[0]):
            ...
            notes_list_b, pedal_events_b = sm_tokenizer.midi_events_to_notes(events_list_b)
